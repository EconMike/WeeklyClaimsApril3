...
a<-ggplot(dfo, aes(x=DATE, y=Claims)) + 
  ...
  scale_x_date(breaks = as.Date(c("2020-03-07", "2021-04-03")),date_breaks = "8 weeks", date_labels = "%b%Y")+
  theme_economist()+
  ...
  annotate(geom = "text", x = as.Date("2021-02-20") , y = 200000, label = "Week Ending\n April 3: 744K",hjust = "Right", vjust=-2)+
  annotate("segment", x = as.Date("2021-04-03"), xend = as.Date("2021-04-03"), y = 1800000, yend = 900000)
...
b<-ggplot(df, aes(x=DATE, y=CC)) + 
  ...
  xlab("") +ylab("")+theme(plot.title=element_text(size = 12), plot.subtitle = element_text(size=10,hjust = 0),
                           axis.text.x = element_text(size = 10))+
  annotate(geom = "text", x = as.Date("2021-02-06") , y = 900000, label = "Week Ending\n Mar 27: 3.7M",hjust = "Right", vjust=-2)+
  annotate("segment", x = as.Date("2021-03-27"), xend = as.Date("2021-03-27"), y = 7000000, yend = 5000000)



plot_grid(a, b)

ggsave("T2woClaimsApril8.png")
